...
public class LoggerAspect {
    ...
    public Object log(ProceedingJoinPoint joinPoint) throws Throwable {
        log.info(joinPoint.toShortString() + " method execution start");
        Instant start = Instant.now();
        Object returnObj = joinPoint.proceed();
        Instant finish = Instant.now();
        long timeElapsed = Duration.between(start, finish).toMillis();
        log.info("Time took to execute " + joinPoint.toShortString()+ " method is : "+timeElapsed);//getSignature().toString()
        log.info(joinPoint.toShortString() + " method execution end");
        return returnObj;
    }
